<template>
  <!-- MOBILE -->
  <nav 
  class="navbar fixed top-0 z-10 md:hidden flex justify-between align-middle w-full p-5 text-nearblack"
  :class="{ 'navbar-hidden' : !showNav, 'bg-nearblack text-white' : showNavBg }">
    ...
    class="flex flex-col justify-end">
      ...
      class="flex flex-col items-end mt-4 space-y-4">
        ...
        <button 
        class="border-2 px-4 py-2 text-sm"
  ...
  <!-- LARGE SCREENS -->
  <nav
  class="navbar fixed top-0 z-10 hidden md:flex items-center w-full px-5 py-8 text-nearblack"
  :class="{ 'navbar-hidden' : !showNav, 'bg-nearblack text-white' : showNavBg }">
    <div class="w-3/4 mx-auto flex justify-between">
      <nav-brand />
      <ul
      class="flex flex-row items-center mt-0 space-x-8">
        <router-link to="/info"
        ...
        Kontakt</router-link>
        <button 
        class="bg-transparent hover:bg-nearblack border-2 px-4 py-2 text-sm border-nearblack"
